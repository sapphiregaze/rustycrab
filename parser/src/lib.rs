...
impl MyParser {
  ...
  pub fn parse(&self) -> Result<SExpr, String> {
    ...
    match myparser().parse(token_stream).into_result() {
...
fn myparser<'tokens, 'src: 'tokens, I>(
) -> impl Parser<'tokens, I, SExpr, extra::Err<Rich<'tokens, LexerToken>>>
where
...
fn main() {
    // let thing = SimpleSpan::new(0..0.into(), 0..0);
    // let other_thing = SimpleSpan::new(0..0.into(), 0..0);
